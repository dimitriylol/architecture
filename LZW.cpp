...
string string_butlast_char(string str) {
	string tmp = str.substr(0, str.length() - 1);
	return (tmp.empty()) ? str : tmp;
}

int LZW::get_bits_size(int val) {
	if (val == 0)
...
void LZW::print_alphabet() {
	for (auto& kvp: alphabet)
		cout << "key " << kvp.first << " val " << kvp.second << endl;
	cout << "Val " << value << endl;
}

void LZW::print_rev_alphabet() {
	for (auto& kvp: reverse_alphabet)
		cout << "key " << kvp.first << " val " << kvp.second << endl;
	cout << "Val " << value << endl;
}
...
string LZW::encode(string to_encode) {
	...
	for (; next_sym != to_encode.end(); next_sym++) {
		...
		cout << "cur str " << current_str;
		if (alphabet.find(current_str) == alphabet.end()) {
//			try {
			cout << " res " << dec2bin(alphabet.at(string_butlast_char(current_str)));
			cout << " str " << alphabet.at(string_butlast_char(current_str)) << endl;
